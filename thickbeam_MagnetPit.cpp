...
#define Bz(C0, z, x) ((1.0 + C0) / 2.0 + ((1 - C0) / 2.0)*cos(2*PI*z))
#define Bx(C0, z, x) (2*PI*((1.0 - C0) / 2.0)*sin(2*PI*z)*x)
...
int main()
{
	...
	char filenamedata[65];
	char filenametw[65];

	float C0, nc, w, Xc, Rho;
	fscanf(params, "%f%f%f%f%f", &C0, &nc, &w, &Xc, &Rho);

	sprintf(filenamedata, "TrajectoryPit_C%2.2f_nc%2.2f_w%2.2f_Xc%2.2f_Rho%2.2f.csv", C0, nc, w, Xc, Rho);
	sprintf(filenametw,    "TimeWidthPit_C%2.2f_nc%2.2f_w%2.2f_Xc%2.2f_Rho%2.2f.csv", C0, nc, w, Xc, Rho);
